...
k1=10
#becuase its PD, k2 must be <0 to implicityly convey the de(n)/dt
k2=-2
...
#input is the current temperature (as decided by the slider)
        if self.target>=inp:
            ...
            outp=(round(k1*diff1+k2*self.diff2,2), round(k1*diff1+k2*self.diff2,2))
...
#building a temperature simulator to set the current temperature        
        self.current_slider = Slider(min=25.0, max=40.0, value=35.0)
